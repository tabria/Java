...
public class BoatSimulatorController implements IBoatSimulatorController {
    ...
    public String GetStatistic() {
        ...
        double powerBoatPercentage = 0.0;
        List<IBoat> boatParticipants = this.currentRace.GetParticipants();
        for (int i = 0; i <boatParticipants.size() ; i++) {
            if (boatParticipants.get(i).getBoatType().contains("Power")){
                powerBoatPercentage++;
            }
        }
        powerBoatPercentage = (powerBoatPercentage / participants) * 100;

        //double powerBoatPercentage = this.currentRace.GetParticipants().stream().filter(p -> p.getBoatType().contains("Power")).count() / participants * 100;

        double rowBoatPercentage = 0.0;
        List<IBoat> rowBoatParticipants = this.currentRace.GetParticipants();
        for (int i = 0; i <rowBoatParticipants.size() ; i++) {
            if (rowBoatParticipants.get(i).getBoatType().contains("Row")){
                rowBoatPercentage++;
            }
        }
        rowBoatPercentage = (rowBoatPercentage / participants) * 100;
        //double rowBoatPercentage = this.currentRace.GetParticipants().stream().filter(p -> p.getBoatType().contains("Row")).count() / participants * 100;

        double sailBoatPercentage = 0.0;
        List<IBoat> sailBoatParticipants = this.currentRace.GetParticipants();
        for (int i = 0; i <sailBoatParticipants.size() ; i++) {
            if (sailBoatParticipants.get(i).getBoatType().contains("Sail")){
                sailBoatPercentage++;
            }
        }
        sailBoatPercentage = (sailBoatPercentage / participants) * 100;
        //double sailBoatPercentage = this.currentRace.GetParticipants().stream().filter(p -> p.getBoatType().contains("Sail")).count() / participants * 100;

        double yachtBoatPercentage = 0.0;
        List<IBoat> yachtBoatParticipants = this.currentRace.GetParticipants();
        for (int i = 0; i <yachtBoatParticipants.size() ; i++) {
            if (yachtBoatParticipants.get(i).getBoatType().contains("Yacht")){
                yachtBoatPercentage++;
            }
        }
        yachtBoatPercentage = (yachtBoatPercentage / participants) * 100;
        //double yachtBoatPercentage = this.currentRace.GetParticipants().stream().filter(p -> p.getBoatType().contains("Yacht")).count() / participants * 100;
